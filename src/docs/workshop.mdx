  ...
  code={`
    ...
    const draggable = ({ state, d }) => (
      <circle
        ...
        r={50}
        fill="blue"
        data-on-drag={() => d.span([{status: "off"}, {status: "on"}])}
      />
    );
...
- Here we use `d.span`, which means "I want to be able to drag back and forth between all the states in this list". There are many alternatives, most prominently `d.floating`, which means the dragged element entirely detaches from the diagram.
